...
public partial class SqlSegmentPreprocessor
{
    ...
    private class TemporaryAliasReference(ISqlReference baseRef, string alias) : ISqlReference, ISqlAliasable
    {
        public string? Alias { get; set; } = alias;
        public string FallbackAlias => baseRef.FallbackAlias; // FIX: Strict non-nullable string
        
        public bool IsAliasQuoted { get; set; } = true;
        public ISqlFragment Source => baseRef.Source; 
        
        public string ToSql(ISqlContext context, SqlRenderMode renderMode = SqlRenderMode.Default) 
            => !string.IsNullOrWhiteSpace(Alias) ? Alias : baseRef.ToSql(context, renderMode);
    }

    private class AliaslessReference(ISqlReference baseRef) : ISqlReference, ISqlAliasable
    {
        public string? Alias 
        { 
            get => null; 
            set { if (baseRef is ISqlAliasable a) a.Alias = value; } 
        }
        ...
        public string ToSql(ISqlContext context, SqlRenderMode renderMode = SqlRenderMode.Default) 
            => baseRef.ToSql(context, renderMode);
    }
}