...
//             var sql = $"UPDATE ({upSub.Subquery.ToSql(context)}) {quotedAlias}{Environment.NewLine}SET {upSub.SetClause.ToSql(context)}";
//             if (upSub.WhereClause != null) sql += $"{Environment.NewLine}WHERE {upSub.WhereClause.ToSql(context)}";
//             return sql;
//         }
