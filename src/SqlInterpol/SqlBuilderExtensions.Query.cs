...
public static partial class SqlBuilderExtensions
{
    extension (SqlBuilder builder)
    {
        ...
        public ISqlQuery Query<T1>(Action<ISqlEntity<T1>> body)
            => Entity<T1>(builder)
            .Query(body);

        public ISqlQuery Query<T1, T2>(Action<ISqlEntity<T1>, ISqlEntity<T2>> body)
            => Entity<T1>(builder)
            .Entity<T2>()
            .Query(body);

        public ISqlQuery Query<T1, T2, T3>(Action<ISqlEntity<T1>, ISqlEntity<T2>, ISqlEntity<T3>> body)
            => Entity<T1>(builder)
            .Entity<T2>()
            .Entity<T3>()
            .Query(body);

        public ISqlQuery Query<T1, T2, T3, T4>(Action<ISqlEntity<T1>, ISqlEntity<T2>, ISqlEntity<T3>, ISqlEntity<T4>> body)
            => Entity<T1>(builder)
            .Entity<T2>()
            .Entity<T3>()
            .Entity<T4>()
        ...
        public ISqlQuery Query<T1, T2, T3, T4, T5>(Action<ISqlEntity<T1>, ISqlEntity<T2>, ISqlEntity<T3>, ISqlEntity<T4>, ISqlEntity<T5>> body)
            => Entity<T1>(builder)
            .Entity<T2>()
            .Entity<T3>()
            .Entity<T4>()
...
public readonly struct SqlQueryBuilder<T1>(SqlBuilder builder, string? name, string? schema)
{
    ...
    {
        return new SqlQueryBuilder<T1, T2>(Builder, Entity1, name, schema);
    }

    public ISqlQuery Query(Action<ISqlEntity<T1>> body)
    {
        var e1 = Entity1;
        return Builder.Query(() => body(e1));
    }
}
...
public readonly struct SqlQueryBuilder<T1, T2>(SqlBuilder builder, ISqlEntity<T1> e1, string? name, string? schema)
{
    ...
    // Instance method chaining to T3
    public SqlQueryBuilder<T1, T2, T3> Entity<T3>(string? name = null, string? schema = null)
    {
        return new SqlQueryBuilder<T1, T2, T3>(Builder, Entity1, Entity2, name, schema);
    }

    public ISqlQuery Query(Action<ISqlEntity<T1>, ISqlEntity<T2>> body)
    {
        var e1 = Entity1;
        var e2 = Entity2;
        return Builder.Query(() => body(e1, e2));
    }
}
...
public readonly struct SqlQueryBuilder<T1, T2, T3>(SqlBuilder builder, ISqlEntity<T1> e1, ISqlEntity<T2> e2, string? name, string? schema)
{
    ...
    public ISqlQuery Query(Action<ISqlEntity<T1>, ISqlEntity<T2>, ISqlEntity<T3>> body)
    {
        var e1 = Entity1;
        var e2 = Entity2;
        var e3 = Entity3;
        return Builder.Query(() => body(e1, e2, e3));
    }
}
...
public readonly struct SqlQueryBuilder<T1, T2, T3, T4>(SqlBuilder builder, ISqlEntity<T1> e1, ISqlEntity<T2> e2, ISqlEntity<T3> e3, string? name, string? schema)
{
    ...
    public ISqlQuery Query(Action<ISqlEntity<T1>, ISqlEntity<T2>, ISqlEntity<T3>, ISqlEntity<T4>> body)
    {
        var e1 = Entity1;
        var e2 = Entity2;
        var e3 = Entity3;
        var e4 = Entity4;
        return Builder.Query(() => body(e1, e2, e3, e4));
    }
}
...
public readonly struct SqlQueryBuilder<T1, T2, T3, T4, T5>(SqlBuilder builder, ISqlEntity<T1> e1, ISqlEntity<T2> e2, ISqlEntity<T3> e3, ISqlEntity<T4> e4, string? name, string? schema)
{
    ...
    public ISqlQuery Query(Action<ISqlEntity<T1>, ISqlEntity<T2>, ISqlEntity<T3>, ISqlEntity<T4>, ISqlEntity<T5>> body)
    {
        var e1 = Entity1;
        var e2 = Entity2;
        var e3 = Entity3;
        var e4 = Entity4;
        var e5 = Entity5;
        return Builder.Query(() => body(e1, e2, e3, e4, e5));
    }
}
