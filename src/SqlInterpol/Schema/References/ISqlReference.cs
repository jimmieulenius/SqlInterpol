...
/// The alias (if present) causes the reference to render with <c>AS "alias"</c> in declaration contexts.
/// </remarks>
public interface ISqlReference : ISqlFragment
{
    ...
    /// <summary>Gets the alias to use when this reference appears in a SELECT projection.</summary>
    string? Alias { get; }

    /// <summary>Gets whether the alias should be wrapped in dialect-specific identifier quotes.</summary>
    bool IsAliasQuoted { get; }
